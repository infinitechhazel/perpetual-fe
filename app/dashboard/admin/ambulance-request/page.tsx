...
export default function AdminAmbulanceRequestsPage() {
  ...
  const getEmergencyBadge = (emergency: string) => {
    ...
    return (
      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${styles[emergency as keyof typeof styles]}`}>
        ...
        <header className="bg-white border-b border-gray-200 px-4 sm:px-6 py-3 sm:py-4 sticky top-0 z-10">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs sm:text-sm text-gray-600 mt-1">Emergency ambulance request management</p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 sm:hidden mb-4">
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3 text-center">
                ...
                <p className="text-lg font-bold text-blue-600">
                  {requests.filter((r) => ["dispatched", "en_route", "arrived"].includes(r.status)).length}
                </p>
              </div>
              <div className="bg-white rounded-lg border border-gray-200 p-3 text-center">
                    ...
                    <div className="inline-block min-w-full align-middle">
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-linear-to-r from-emerald-600 to-orange-500 text-white">
                          <tr>
                            <th className="px-3 sm:px-4 py-3 text-left text-xs font-semibold uppercase whitespace-nowrap">Request ID</th>
                            ...
                            <th className="px-3 sm:px-4 py-3 text-left text-xs font-semibold uppercase whitespace-nowrap">Emergency Type</th>
                            <th className="px-3 sm:px-4 py-3 text-left text-xs font-semibold uppercase whitespace-nowrap">Location</th>
                            <th className="px-3 sm:px-4 py-3 text-left text-xs font-semibold uppercase whitespace-nowrap">Status</th>
                            <th className="px-3 sm:px-4 py-3 text-left text-xs font-semibold uppercase whitespace-nowrap">Requested</th>
                          ...
                          {requests.map((request) => (
                            <tr key={request.id} className="hover:bg-gray-50 transition-colors">
                              <td className="px-3 sm:px-4 py-3 text-sm font-medium text-gray-900 whitespace-nowrap">
                                <div className="font-mono">{request.request_id}</div>
                              </td>
                              <td className="px-3 sm:px-4 py-3 text-sm text-gray-900 whitespace-nowrap">
                                <div className="max-w-[150px] truncate" title={request.name}>
                                  {request.name}
                                </div>
                              </td>
              ...
              <div style={{ background: 'linear-gradient(to right, rgb(5, 150, 105), rgb(249, 115, 22))' }} className="text-white px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between sticky top-0 z-10">
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h2 className="text-lg sm:text-xl font-bold truncate">Ambulance Request</h2>
                    <p className="text-xs sm:text-sm text-white/90 font-mono">{selectedRequest.request_id}</p>
                  </div>
                </div>
                <button
                  onClick={closeModal}
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 overscroll-contain">
                <div className="space-y-4 sm:space-y-6 pb-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div className="bg-gray-50 rounded-lg p-3 sm:p-4 border border-gray-200">
                      <span className="text-sm font-medium text-gray-700 block mb-2">Emergency Type</span>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        ...
                        <p className="text-sm sm:text-base text-gray-900 font-medium break-words">{selectedRequest.name}</p>
                      </div>
                      <div>
                        <label className="text-xs sm:text-sm font-medium text-gray-500 flex items-center gap-1">
                        ...
                        <a href={`tel:${selectedRequest.phone}`} className="text-sm sm:text-base text-emerald-600 hover:text-emerald-700 font-medium">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-sm sm:text-base text-gray-900 break-words">{selectedRequest.address}</p>
                      </div>
                     {selectedRequest.latitude != null && selectedRequest.longitude != null && (
                        <div>
                          <label className="text-xs sm:text-sm font-medium text-gray-500">GPS Coordinates</label>
                  ...
                  {selectedRequest.notes && (
                    <div>
                      ...
                      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 sm:p-4">
                        <p className="text-sm sm:text-base text-gray-900 break-words">{selectedRequest.notes}</p>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="space-y-2">
                      <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-600">{formatDate(selectedRequest.requested_at)}</p>
                          {selectedRequest.estimated_arrival && (
                            <p className="text-xs text-orange-600 mt-1">ETA: {selectedRequest.estimated_arrival}</p>
                          )}
                        </div>
                      </div>
                      ...
                      {selectedRequest.dispatched_at && (
                        <div className="flex items-start gap-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-600">{formatDate(selectedRequest.dispatched_at)}</p>
                          </div>
                        </div>
                      )}
                      ...
                      {selectedRequest.arrived_at && (
                        <div className="flex items-start gap-3 p-3 bg-cyan-50 rounded-lg border border-cyan-200">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-600">{formatDate(selectedRequest.arrived_at)}</p>
                          </div>
                        </div>
                      )}
                      ...
                      {selectedRequest.completed_at && (
                        <div className="flex items-start gap-3 p-3 bg-green-50 rounded-lg border border-green-200">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-600">{formatDate(selectedRequest.completed_at)}</p>
                          </div>
                        </div>
                      )}
                    </div>
                ...
                <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 sm:gap-3 w-full">
                  {selectedRequest.status === "pending" ? (
                    ...
                    <>
                      ...
                      <button 
                        onClick={() => handleUpdateStatus(selectedRequest.id, "en_route")}
                    ...
                    <>
                      ...
                      <button 
                        onClick={() => handleUpdateStatus(selectedRequest.id, "arrived")}

...
export default function AdminBarangayClearancePage() {
  ...
  const [selectedClearance, setSelectedClearance] = useState<BarangayClearance | null>(null)
  ...
  const fetchClearances = async () => {
    try {
      ...
      const response = await fetch(`/api/barangay-clearance?${params}`, {
        credentials: "include",
      })
      ...
      if (response.ok) {
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: "Failed to fetch barangay clearance applications.",
        })
      }
    } catch (error) {
  ...
  const handleUpdateStatus = async (id: number, newStatus: "approved" | "rejected") => {
    try {
      ...
      const response = await fetch(`/api/barangay-clearance/${id}/status`, {
        method: "PATCH",
        headers: {
      ...
      if (data.success) {
        toast({
          title: "Success",
          description: `Barangay clearance ${newStatus === "approved" ? "approved" : "rejected"} successfully.`,
        })
      ...
      <div className="h-full overflow-auto bg-gray-50">
        ...
        <header className="bg-white border-b border-gray-200 px-4 sm:px-6 py-3 sm:py-4 sticky top-0 z-10">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Barangay Clearance Applications</h1>
              <p className="text-xs sm:text-sm text-gray-600 mt-1">Manage and review clearance requests</p>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-4 mb-4">
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <div className="flex-1 relative">
                  ...
                  <input
                    type="text"
                    placeholder="Search by name, reference number, purpose..."
                    value={searchQuery}
                    ...
                    <div className="inline-block min-w-full align-middle">
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-linear-to-r from-emerald-600 to-orange-500 text-white">
                          <tr>
                            <th className="px-3 sm:px-4 py-3 text-left text-xs font-semibold uppercase whitespace-nowrap">Reference No.</th>
                            <th className="px-3 sm:px-4 py-3 text-left text-xs font-semibold uppercase whitespace-nowrap">Applicant Name</th>
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                          {clearances.map((clearance) => (
                            <tr key={clearance.id} className="hover:bg-gray-50 transition-colors">
                              <td className="px-3 sm:px-4 py-3 text-sm font-medium text-gray-900 whitespace-nowrap">
                                {clearance.reference_number}
                              </td>
                              <td className="px-3 sm:px-4 py-3 text-sm font-medium text-gray-900 whitespace-nowrap">
                                <div className="max-w-[150px] truncate" title={clearance.full_name}>
                                  {clearance.full_name}
                                </div>
                              </td>
                              <td className="px-3 sm:px-4 py-3 text-sm text-gray-600 whitespace-nowrap">
                                <div>{clearance.phone}</div>
                                <div className="text-xs text-gray-500 max-w-[150px] truncate" title={clearance.email}>
                                  {clearance.email}
                                </div>
                              </td>
                              <td className="px-3 sm:px-4 py-3 text-sm text-gray-600 whitespace-nowrap">
                              ...
                              <td className="px-3 sm:px-4 py-3 text-sm text-gray-600 whitespace-nowrap">
                                <div className="max-w-[120px] truncate" title={clearance.purpose}>
                                  {clearance.purpose}
                                </div>
                              </td>
              ...
              <div className="bg-linear-to-r from-emerald-600 to-orange-500 text-white px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between sticky top-0 z-10">
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h2 className="text-lg sm:text-xl font-bold truncate">Barangay Clearance Details</h2>
                    <p className="text-xs sm:text-sm text-white/90">Ref: {selectedClearance.reference_number}</p>
                  </div>
                </div>
                <button
                  onClick={closeModal}
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 overscroll-contain">
                <div className="space-y-4 sm:space-y-6 pb-4">
                  ...
                  <div className="bg-gray-50 rounded-lg p-3 sm:p-4 border border-gray-200">
                    ...
                    {selectedClearance.rejection_reason && (
                    ...
                    {selectedClearance.clearance_number && (
                      <div className="mt-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                        <p className="text-sm text-green-800">
                          <span className="font-medium">Clearance Number: </span>
                          {selectedClearance.clearance_number}
                        </p>
                      </div>
                    )}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        ...
                        <p className="text-sm sm:text-base text-gray-900 font-medium break-words">{selectedClearance.full_name}</p>
                      </div>
                      <div>
                        <label className="text-xs sm:text-sm font-medium text-gray-500">Date of Birth</label>
                        <p className="text-sm sm:text-base text-gray-900">{formatDate(selectedClearance.birth_date)}</p>
                      </div>
                      <div>
                        <label className="text-xs sm:text-sm font-medium text-gray-500">Age</label>
                        <p className="text-sm sm:text-base text-gray-900">{selectedClearance.age} years old</p>
                      </div>
                      <div>
                        <label className="text-xs sm:text-sm font-medium text-gray-500">Sex</label>
                        <p className="text-sm sm:text-base text-gray-900 capitalize">{selectedClearance.sex}</p>
                      </div>
                      <div>
                        <label className="text-xs sm:text-sm font-medium text-gray-500">Civil Status</label>
                        <p className="text-sm sm:text-base text-gray-900 capitalize">{selectedClearance.civil_status}</p>
                      </div>
                      <div>
                        <label className="text-xs sm:text-sm font-medium text-gray-500">Years of Residency</label>
                        <p className="text-sm sm:text-base text-gray-900">{selectedClearance.years_of_residency} years</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        ...
                        <p className="text-sm sm:text-base text-gray-900">{selectedClearance.phone}</p>
                      </div>
                      <div>
                        <label className="text-xs sm:text-sm font-medium text-gray-500 flex items-center gap-1">
                        ...
                        <p className="text-sm sm:text-base text-gray-900 break-all">{selectedClearance.email}</p>
                      </div>
                      <div className="sm:col-span-2">
                        ...
                        <p className="text-sm sm:text-base text-gray-900 break-words">{selectedClearance.address}</p>
                      </div>
                      <div>
                        <label className="text-xs sm:text-sm font-medium text-gray-500">Barangay</label>
                        <p className="text-sm sm:text-base text-gray-900">{selectedClearance.barangay}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="bg-gray-50 rounded-lg p-3 sm:p-4 border border-gray-200">
                      <p className="text-sm sm:text-base text-gray-900 break-words">{selectedClearance.purpose}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="border rounded-lg p-3 bg-gray-50">
                      ...
                      {selectedClearance.valid_id_path ? (
                        <a
                          href={`${process.env.NEXT_PUBLIC_IMAGE_URL}/storage/${selectedClearance.valid_id_path}`}
                          target="_blank"
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        ...
                        <p className="text-sm sm:text-base text-gray-900">{formatDate(selectedClearance.created_at)}</p>
                      </div>
                      {selectedClearance.approved_at && (
                        <div>
                          <label className="text-xs sm:text-sm font-medium text-gray-500">Approved Date</label>
                          <p className="text-sm sm:text-base text-gray-900">{formatDate(selectedClearance.approved_at)}</p>
                        </div>
                      )}
                      {selectedClearance.expires_at && (
                        <div>
                          <label className="text-xs sm:text-sm font-medium text-gray-500">Expiration Date</label>
                          <p className="text-sm sm:text-base text-gray-900">{formatDate(selectedClearance.expires_at)}</p>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 sm:gap-3 w-full">
                  {selectedClearance.status === "pending" ? (
                    <>
                      ...
                      <button 
                        onClick={() => handleUpdateStatus(selectedClearance.id, "rejected")}
                      ...
                      <button 
                        onClick={() => handleUpdateStatus(selectedClearance.id, "approved")}

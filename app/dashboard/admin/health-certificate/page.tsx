...
export default function AdminHealthCertificatePage() {
  ...
  const [selectedCertificate, setSelectedCertificate] = useState<HealthCertificate | null>(null)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [isActionModalOpen, setIsActionModalOpen] = useState(false)
  const [actionType, setActionType] = useState<"approved" | "rejected" | "under_review" | "completed" | null>(null)
  ...
  const fetchCertificates = async () => {
    try {
      ...
      const response = await fetch(`/api/health-certificate?${params}`, {
        credentials: "include",
      })
      ...
      if (response.ok) {
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: "Failed to fetch health certificate applications.",
        })
      }
    } catch (error) {
  ...
  const handleViewCertificate = (certificate: HealthCertificate) => {
  ...
  const openActionModal = (type: "approved" | "rejected" | "under_review" | "completed") => {
    setActionType(type)
    setRemarks("")
  ...
  const handleUpdateStatus = async () => {
    if (!selectedCertificate || !actionType) return
    ...
    try {
      ...
      const response = await fetch(`/api/health-certificate/${selectedCertificate.id}`, {
        method: "PUT",
        headers: {
      ...
      if (data.success) {
        toast({
          title: "Success",
          description: `Health certificate ${actionType} successfully.`,
        })
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: "Failed to update health certificate status.",
      })
    }
  }
  ...
  const getStatusBadge = (status: string) => {
    ...
    const icons = {
      ...
      under_review: <Activity className="w-3 h-3" />,
      ...
      <div className="h-full overflow-auto bg-gray-50">
        ...
        <header className="bg-white border-b border-gray-200 px-4 sm:px-6 py-3 sm:py-4 sticky top-0 z-10">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Health Certificate Applications</h1>
              <p className="text-xs sm:text-sm text-gray-600 mt-1">Manage and review health certificate requests</p>
            </div>
                      ...
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-linear-to-r from-emerald-600 to-orange-500 text-white">
                          <tr>
                            ...
                            <th className="px-3 sm:px-4 py-3 text-left text-xs font-semibold uppercase whitespace-nowrap">Age/Sex</th>
                            <th className="px-3 sm:px-4 py-3 text-left text-xs font-semibold uppercase whitespace-nowrap">Purpose</th>
                          ...
                          {certificates.map((certificate) => (
                            <tr key={certificate.id} className="hover:bg-gray-50 transition-colors">
                              <td className="px-3 sm:px-4 py-3 text-sm font-medium text-gray-900 whitespace-nowrap">
                                <div className="max-w-[120px] truncate" title={certificate.reference_number}>
                                  {certificate.reference_number}
                                </div>
                              </td>
                              <td className="px-3 sm:px-4 py-3 text-sm font-medium text-gray-900 whitespace-nowrap">
                                <div className="max-w-[150px] truncate" title={certificate.full_name}>
                                  {certificate.full_name}
                                </div>
                              </td>
                              <td className="px-3 sm:px-4 py-3 text-sm text-gray-600 whitespace-nowrap">
                                <div className="max-w-[150px] truncate" title={certificate.email}>
                                  {certificate.email}
                                </div>
                              </td>
                              <td className="px-3 sm:px-4 py-3 text-sm text-gray-600 whitespace-nowrap">
                              ...
                              <td className="px-3 sm:px-4 py-3 text-sm text-gray-600 whitespace-nowrap">
                                {certificate.age} / {certificate.sex === "male" ? "M" : "F"}
                              </td>
                              <td className="px-3 sm:px-4 py-3 text-sm text-gray-600 whitespace-nowrap">
                                <div className="max-w-[120px] truncate" title={certificate.purpose}>
                                  {certificate.purpose}
                                </div>
                              </td>
            ...
            <div className="bg-white sm:rounded-xl shadow-2xl w-full sm:max-w-4xl h-[95vh] sm:h-auto sm:max-h-[90vh] overflow-hidden flex flex-col">
              ...
              <div className="bg-linear-to-r from-emerald-600 to-orange-500 text-white px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between sticky top-0 z-10">
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h2 className="text-lg sm:text-xl font-bold truncate">Health Certificate Details</h2>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        ...
                        <p className="text-sm sm:text-base text-gray-900 font-medium break-words">{selectedCertificate.full_name}</p>
                      </div>
                      <div>
                        ...
                        <p className="text-sm sm:text-base text-gray-900 break-all">{selectedCertificate.email}</p>
                      </div>
                      <div>
                        <label className="text-xs sm:text-sm font-medium text-gray-500 flex items-center gap-1">
                      ...
                      <div>
                        ...
                        <p className="text-sm sm:text-base text-gray-900">{formatDate(selectedCertificate.birth_date)}</p>
                      </div>
                      <div>
                      ...
                      <div className="sm:col-span-2">
                        ...
                        <p className="text-sm sm:text-base text-gray-900 break-words">{selectedCertificate.address}</p>
                      </div>
                      <div className="sm:col-span-2">
                        <label className="text-xs sm:text-sm font-medium text-gray-500">Purpose</label>
                        <p className="text-sm sm:text-base text-gray-900 break-words">{selectedCertificate.purpose}</p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1">
                            ...
                            {selectedCertificate.has_allergies ? (
                              <p className="text-sm text-gray-900 mt-1">{selectedCertificate.allergies || "Not specified"}</p>
                            ) : (
                      ...
                      <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1">
                            <label className="text-xs sm:text-sm font-medium text-gray-700">Current Medications</label>
                            {selectedCertificate.has_medications ? (
                              <p className="text-sm text-gray-900 mt-1">{selectedCertificate.medications || "Not specified"}</p>
                            ) : (
                              <p className="text-sm text-gray-500 mt-1">No current medications</p>
                            )}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1">
                            <label className="text-xs sm:text-sm font-medium text-gray-700">Medical Conditions</label>
                            {selectedCertificate.has_conditions ? (
                              <p className="text-sm text-gray-900 mt-1">{selectedCertificate.conditions || "Not specified"}</p>
                            ) : (
                              <p className="text-sm text-gray-500 mt-1">No known medical conditions</p>
                            )}
                          </div>
                        </div>
                      </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        ...
                        <p className="text-sm sm:text-base text-gray-900">{formatDate(selectedCertificate.created_at)}</p>
                      </div>
                      <div>
                        <label className="text-xs sm:text-sm font-medium text-gray-500">Last Updated</label>
                        <p className="text-sm sm:text-base text-gray-900">{formatDate(selectedCertificate.updated_at)}</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="border-t border-gray-200 px-4 sm:px-6 py-3 bg-white shadow-lg flex-shrink-0">
                <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 sm:gap-3 w-full">
                  {selectedCertificate.status === "pending" || selectedCertificate.status === "under_review" ? (
                    <>
                      <button
                        onClick={closeModal}
                    ...
                    <>
                      ...
                      <button 
                        onClick={() => openActionModal("completed")}
        ...
        {isActionModalOpen && actionType && selectedCertificate && (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
              {/* Modal Header */}
              <div className={`px-6 py-4 border-b border-gray-200 ${
                actionType === "approved" ? "bg-green-50" :
                actionType === "rejected" ? "bg-red-50" :
                actionType === "completed" ? "bg-purple-50" :
                "bg-blue-50"
              }`}>
                <h3 className={`text-lg font-semibold ${
                  actionType === "approved" ? "text-green-900" :
                  actionType === "rejected" ? "text-red-900" :
                  actionType === "completed" ? "text-purple-900" :
                  "text-blue-900"
                }`}>
                ...
                <p className="text-sm text-gray-600 mt-1">
                  {selectedCertificate.full_name} - {selectedCertificate.reference_number}
                </p>
              </div>
              ...
              <div className="p-6">
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {actionType === "rejected" ? "Reason for Rejection *" : "Remarks (Optional)"}
                  </label>
                  <textarea
                    value={remarks}
                ...
                <div className={`p-3 rounded-lg text-sm ${
                  actionType === "approved" ? "bg-green-50 text-green-800" :
                  actionType === "rejected" ? "bg-red-50 text-red-800" :
                  actionType === "completed" ? "bg-purple-50 text-purple-800" :
                  "bg-blue-50 text-blue-800"
                }`}>
                  <p className="font-medium">
                    {actionType === "approved" ? "This will approve the health certificate application." :
                     actionType === "rejected" ? "This will reject the health certificate application." :
                     actionType === "completed" ? "This will mark the certificate as completed and issued." :
                     "This will move the application to under review status."}
                  </p>
                </div>
              </div>
              ...
              <div className="px-6 py-4 bg-gray-50 rounded-b-xl flex gap-3">
                ...
                <button
                  ...
                  className={`flex-1 px-4 py-2 text-white rounded-lg transition-colors text-sm font-medium ${
                    ...
                    actionType === "completed" ? "bg-purple-600 hover:bg-purple-700" :
